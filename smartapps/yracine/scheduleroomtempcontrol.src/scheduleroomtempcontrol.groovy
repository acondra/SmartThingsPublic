...
def get_APP_VERSION() {return "4.3.1"}
...
void open_vents_in_zone(indiceZone) {
	def key= "zoneName$indiceZone"    
	def zoneName= settings[key]
	traceEvent(settings.logFilter,"open_vents_in_zone>opening all vents in $zoneName",settings.detailedNotif, get_LOG_INFO())
	...
	if (tempSensor != null) {
		...
		if ((refreshSensor) && (tempSensor.hasCapability("Refresh"))) {
			...
			try {        
				...
				traceEvent(settings.logFilter,"getSensorTemperature>not able to do a refresh() on $tempSensor",settings.detailedNotif, get_LOG_INFO())
			}                
		}        
...
private void refresh_virtual_zone_values(indiceZone,refreshSensors=false) {
	def tempAtSensor
	def key = "zoneName$indiceZone"
	def zoneName= settings[key]
	String mode = 'auto'
	...
	for (room in rooms) {
		...
		if ((refreshSensors) && ((tempSensor) && (tempSensor.hasCapability("Refresh")))) {
			...
			try {        
				...
				traceEvent(settings.logFilter,"refresh_virtual_zone_values>not able to do a refresh() on $tempSensor",settings.detailedNotif, get_LOG_INFO())
			}                
		}        
		key = "motionSensor$indiceRoom"
		def motionSensor = settings[key]
		if (motionSensor != null) {
			...
			if ((refreshSensors) && ((motionSensor != tempSensor) && (motionSensor.hasCapability("Refresh")))) {
				...
				try {        
					...
					traceEvent(settings.logFilter,"refresh_virtual_zone_values>not able to do a refresh() on $motionSensor",settings.detailedNotif, get_LOG_INFO())
				}                
			}
...
// Refresh My Virtual zone device if needed    
	...
	if (virtualCreateZoneFlag) {
		...
		if (d) {  
			...
			isChange = d.isStateChange(d, "activeInSchedule", "on")	//check of state change
			d.sendEvent(name:"activeInSchedule", value: "on", isDisplayed: true, isStateChange: isChange)          
			...
			if (thermostat) {            
				...
				d.sendEvent(name:"thermostatSetpoint", value: tempValueString, isDisplayed:true)
			} /* end if thermostat */
			...
			if (mode in ['heat', 'off','auto'] && (tempDelta != null) && (tempDelta!=desiredHeatDelta)) {
			...
			} else if ((desiredHeatDelta) && (mode in ['heat', 'off','auto'])) {
				d.sendEvent(name:"tempDelta", value: desiredHeatDelta, isDisplayed:true)
			}  // the delta in virtual zone takes precedence over the ones set in the smartapp          
			if (mode in ['cool', 'off','auto'] && (tempDelta != null) && (tempDelta!=desiredCoolDelta))  {
			...
			} else if ((desiredCoolDelta) && (mode in ['cool', 'off','auto'])) {
				d.sendEvent(name:"tempDelta", value: desiredCoolDelta, isDisplayed:true)
			}            
...
private def setVentSwitchLevel(indiceRoom, ventSwitch, switchLevel=100) {
	...
	try {
		...
		if (roomName) {       
			traceEvent(settings.logFilter,"set ${ventSwitch} to level ${switchLevel} in room ${roomName} to reach desired temperature",settings.detailedNotif, get_LOG_INFO())
		}            
	} catch (e) {
		if (switchLevel >0) {
...
def IsRightDayForChange(indiceSchedule) {
	...
	traceEvent(settings.logFilter, "IsRightDayForChange>schedule $scheduleName, currentDayOfWeek=$currentDay vs. dayOfWeek from Schedule $indiceSchedule=$dayOfWeek", 
		settings.detailedNotif, get_LOG_TRACE())  
