...
def get_APP_VERSION() {return "8.4.2"}
	...
	if (settings.doNotUseHumTableFlag) {    
		...
		if ((currentMode in ['cool','off', 'auto', 'eco']) && ((outdoorTemp) &&
			...
			if (settings."setVentLevel${indiceSchedule}"==null) {
				...
				def settingKey= "setVentLevel${indiceSchedule}"   
				app.updateSetting(settingKey,SET_LEVEL_BYPASS)        
			...
			if (settings."setVentLevel${indiceSchedule}"== SET_LEVEL_BYPASS) {          
				...
				def settingKey= "setVentLevel${indiceSchedule}"   
				app.updateSetting(settingKey,null)        
...
/* Commented out to avoid any "offline" issues on some sensors following some ST platform changes. */
	...
	if (!foundSchedule) {
		...
		if (evaporativeCoolerSwitch) {
			traceEvent(settings.logFilter,"setZoneSettings>about to turn off switch(es)",settings.detailedNotif, get_LOG_INFO())
			evaporativeCoolerSwitch.off() // Turn off the alternative cooling for the running schedule 
			restore_thermostat_mode()
		}
	} 
				...
				if (isRoomOccupied(motionSensor, indiceRoom)) {
					...
					if (roomTstat) {
						...
						if ((settings.turnOffRoomTstatsFlag) && ((!currentMode in ['off', 'eco']))) {
							traceEvent(settings.logFilter,"setAllRoomTstatsSettings>schedule ${scheduleName},$roomTstat to send eco or off as turnOffRoomTstatsFlag=true",
								settings.detailedNotif)            
...
*/
		...
		if (!found_room_in_zone) { 	// find the zone(s) associated to the excluded room	    
			...
			if (settings.turnOffRoomTstatsFlag) {    
				...
				traceEvent(settings.logFilter,"turn_off_room_tstats_outside_zones>zone=${zone}: zoneDetails= ${zoneDetails}, found $roomTstat in $roomName",settings.detailedNotif)
